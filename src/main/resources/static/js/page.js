	...
	$.fn.wrpage = function(options) {
		...
		function init(size, cur) {
			...
			if (cur <= 1) {
				...
				_html +=
					'<a onclick="refresh(1)" class="wrpage wr_pagefirst">首页</a><a onclick="refresh("+size+")" class="wrpage wr_pagepre">上一页</a>'
			}
			//中间
			if (cur < 5) {
				for (var i = 1; i <= 6; i++) {
					if (i == cur) {
						...
						_html += '<a onclick="refresh('+i+')" class="wrpage wrpage_number">' + i + '</a>'
					}
				}
			} else if (cur >= 5) {
					...
					if (size == 6) {
						...
						for (var i = 2; i <= size; i++) {
							if (i == cur) {
								...
								_html += '<a onclick="refresh("+i+")" href="javascript:;" class="wrpage wrpage_number">' + i + '</a>'
							}
						}
					} else {
			...
			if (cur >= size) {
				...
				_html +=
					'<a onclick="refresh('+size+')" href="javascript:;" class="wrpage wr_pagenext">下一页</a>' +
					'<a onclick="refresh('+size+')" href="javascript:;" class="wrpage wr_pageend">尾页</a>'
			}
			em.append(_html)
		}
