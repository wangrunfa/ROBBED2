...
Basicmanager particularsMessage(@Param("particularsId")Integer particularsId);
    ...
    @Select(" select count(*) from  qd_basicmanager where qd_phone=#{phone} limit 1")
    Integer findPhones(@Param("phone")String phone);
...
//    Basicmanager GrabASingleListYidu(@Param("searchConditions") SearchCondition searchCondition,@Param("getLgRead") Integer getLgRead);
    ...
    //查询是否购买 的價格
    @Select("select lg_gm_pays from qd_lg_power where lg_phone=#{lgPhone} and lg_shop_uid=#{gmid}")
    Integer whetherPurchasesssssss(@Param("lgPhone")String lgPhone,@Param("gmid") Integer gmid);
    //查询是否已读
    ...
    @Insert(" INSERT INTO\n" +
            "            qd_lg_power(lg_phone,lg_ztc_id,lg_gm_pays,lg_qd_id,lg_addtime)\n" +
            "            values(#{lgPhone},#{particularsId},#{gmpays},#{qdid},NOW());")
...
//    void updateBalancejiage(@Param("pays")Integer pays);
    ...
    @Select("select id,qd_qdname,qd_qdurl,qd_addvartime,qd_id,qd_klbfb from qd_qdtj where login_name=#{findQdMessageModel.loginusername} and login_password=#{findQdMessageModel.loginpassword} LIMIT 1")
    QdTj findqdmessagedataajaxsImpl(@Param("findQdMessageModel")FindQdMessageModel findQdMessageModel);

    List<FindQdMessageEverydayNumber> findqdmessageEverydayNumberImpl(@Param("findQdMessageModel")FindQdMessageModel findQdMessageModel, @Param("id") Integer id);
    ...
    @Select("select * from qd_qdtj where qd_id=#{sourceId}")
    QdTj findQdTjId(@Param("sourceId")String sourceId);
    ...
    Integer findQdPvUv(@Param("sourceId")Integer sourceId);
    ...
    @Select("select * from qd_qdtj where qd_id=#{qdSource} limit 1")
    QdTj findqdtjstatus(@Param("qdSource")String qdSource);
    ...
    void addQdJztStatus(@Param("lgShopUid")Integer lgShopUid,@Param("qdId") Integer qdId);

    Integer dicectDriveListzongshu(@Param("searchConditions")SearchCondition searchCondition,@Param("privatePersonalSubscriptionsModel")PersonalSubscriptionsModel privatePersonalZTCSubscriptionsModel);

    Integer purchaseListszongshu(@Param("searchConditions") SearchCondition searchCondition);
