...
public class SmsSampleUtil {
    ...
    public static Map<String, Object> smsSample(String lgphone) {
        //验证码数量

     //验证码随机数

    String testUsername = "wofx"; //在短信宝注册的用户名
    String testPassword = "cwjmecijwejmVENUIW2"; //在短信宝注册的密码
    String testPhone = lgphone;
    String testContent = "【好多鱼】您的验证码是"+verificationCode+",５分钟内有效。若非本人操作请忽略此消息。"; // 注意测试时，也请带上公司简称或网站签名，发送正规内容短信。千万不要发送无意义的内容：例如 测一下、您好。否则可能会收不到
    ...
    int result = Integer.parseInt(request(httpUrl, httpArg.toString()));
        System.out.println(result);
        if(result==0){
            return WebTools.returnData(verificationCode,0);
        }
        return WebTools.returnData("短信错误"+result,1);
